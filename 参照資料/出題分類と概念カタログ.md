...
- 日常の作問では、`briefing` の採用候補と必要な関連語・直接前提の行だけを参照する。全読みに戻す条件は `skills/security-specialist-trainer/references/カタログ部分参照.md` を正本とする。

このカタログは、IPAの[情報処理安全確保支援士試験情報](https://www.ipa.go.jp/shiken/kubun/sc.html)、[シラバス Ver.2.1](https://www.ipa.go.jp/shiken/syllabus/nq6ept00000014ir-att/syllabus_sc_ver2_1.pdf)、[令和6年度](https://www.ipa.go.jp/shiken/mondai-kaiotu/2024r06.html)・[令和7年度](https://www.ipa.go.jp/shiken/mondai-kaiotu/2025r07.html)の公開過去問を基準に、2026-08-28時点で156語へ拡張した。過去問PDFの分析結果は[過去問分析索引](過去問分析索引.md)へ分離して保持する。日々の出題はこのカタログと`進捗/`だけを用いる。PDFの確認やカタログへの語句追加は、利用者がカタログ拡張または過去問を踏まえた用語追加を明示した場合だけ行う。これは余裕をもって合格圏の基礎語彙をカバーするための出発点であり、午後問題の読解・記述演習そのものを置き換えるものではない。
...
| IDベースアクセス制御（IBAC） | 認証・認可 / IAM | A/B | 3 | 2 | no | IAM / 認証と認可の分離 | user identity / permission assignment / ACL |
...
| WebAuthn / パスキー | 認証・認可 / IAM | B | 4 | 3 | no | 公開鍵暗号 / 多要素認証 | フィッシング耐性 / RP ID / FIDO2 / CTAP / client / authenticator |
| パスワードポリシーとアカウントロック | 認証・認可 / IAM | A/B | 4 | 2 | no | パスワード認証 | ブルートフォース / rate limit / MFA |
| クレデンシャルスタッフィング | 認証・認可 / IAM | B | 4 | 3 | no | パスワード認証 / 認証と認可の分離 | 漏えいパスワード / rate limit / MFA / パスワードスプレー |
...
| EAP-TLS | ネットワークセキュリティ | B | 4 | 3 | no | ネットワークアクセス制御（NAC） / 証明書チェーン検証 | 802.1X / RADIUS / PEAP / クライアント証明書認証 |
| RADIUS | ネットワークセキュリティ | A/B | 3 | 3 | no | ネットワークアクセス制御（NAC） | AAA / 802.1X / EAP-TLS / Diameter / 認証VLAN |
| DHCPスヌーピング | ネットワークセキュリティ | B | 3 | 3 | no | ネットワークセグメンテーション / ファイアウォールと状態管理 | rogue DHCP / IP source guard / Dynamic ARP Inspection |
| NATとポートフォワーディング | ネットワークセキュリティ | A/B | 3 | 2 | no | IPアドレス / ポート | 内部アドレス / 公開サービス / 到達性 |
| 無線LANセキュリティ | ネットワークセキュリティ | B | 4 | 3 | no | 無線LAN / 認証 | WEP / WPA / WPA2 / WPA3 / TKIP / CCMP / SAE / 802.1X / rogue AP |
| WPA2・WPA3 | ネットワークセキュリティ | B | 4 | 3 | no | 無線LANセキュリティ | WEP / WPA / TKIP / CCMP / SAE / 802.1X |
...
| ゼロトラスト型アクセス制御 | ゼロトラスト | A/B | 4 | 3 | no | ゼロトラスト / IAM / 最小権限 | RBAC / ABAC / context / risk / 継続的認可 |
