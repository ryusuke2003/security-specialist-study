...
問題作成後と採点後には、次のコマンドで`学習記録/未解答一覧.md`を更新する。この一覧には、標準のHTMLコメントだけが残る回答欄を未解答として表示し、`わかりません`などの記入済み回答は表示しない。各行全体を元のSessionファイルへのリンクにする。
...
`Mode: quick-review` のSessionは既定8問で、各問にA/B/Cの選択肢を三つだけ置く。回答欄には選んだ文字だけを記入する。`a` / `b` / `c` は前後空白を除いてA/B/Cとして扱う。採点時は正解なら `100 / 100`、不正解なら `0 / 100` とし、短い解説を付ける。
...
「採点して」では、現在・旧形式を含む全SessionのMarkdown本文を走査し、**全回答が記入済みで、かつ採点とprogress更新が完了していないSessionをすべて**対象にする。回答の有無は各 `### 回答` 欄を直接確認して決め、`git diff`、コミット履歴、作業ツリーの状態を判定材料にしない。回答欄の標準プレースホルダは空欄として扱い、`わかりません` は回答済みとして扱う。`Status`の値で対象を選ばないため、全回答を記入したまま`awaiting_answers`が残っていても対象になる。
