...
`Mode: quick-review` のSessionは既定8問で、各問にA/B/Cの選択肢を三つだけ、`- [ ] A. ...` のMarkdownタスクリスト形式で置く。学習者は正解だと思う選択肢を一つだけチェックする。`### 回答`欄や、チェックボックスで回答する旨の案内は置かない。採点時はチェック済みの選択肢が一つなら、正解を `100 / 100`、不正解を `0 / 100` とし、短い解説を付ける。未チェックまたは複数チェックは未回答として扱う。
