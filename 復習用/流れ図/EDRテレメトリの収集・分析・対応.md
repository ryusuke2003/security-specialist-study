...
sequenceDiagram
    ...
    O-->>E: OSがプロセス起動・ファイル変更・通信等のイベントを通知する
    E->>E: エージェントが端末ID・時刻・プロセス親子関係等を付ける
    E->>P: エージェントが保護された通信でテレメトリを送る
    P->>P: 分析基盤が受信データを保存し、ルール・振る舞いで相関分析する
    alt 分析基盤が不審な挙動を検知した
        P-->>A: 分析基盤が対象端末・時系列・検知根拠を表示する
        A->>P: SOC担当者が端末ID・ハッシュ・時刻等で関連データを検索する
...
sequenceDiagram
    ...
    A->>P: SOC担当者が対象端末と隔離・プロセス停止等の操作を指定する
    ...
    P->>P: 分析基盤が指示者・指示内容・確認できた結果を監査ログへ保存する
```
